        ...
        try {
            if ($this->statusCode !== 403) {
                throw new \InvalidArgumentException(
                    'Sierrha-StatusForbiddenHandler only handles HTTP status 403.',
                    1547651137
                );
            }
            if (empty($this->handlerConfiguration['tx_sierrha_loginPage'])) {
                throw new \InvalidArgumentException(
                    'Sierrha-StatusForbiddenHandler needs to have a login page URL set.', 1547651257
                );
            }
            if ($request->getHeader('x-sierrha')) {
                throw new \InvalidArgumentException(
                    'Sierrha-StatusForbiddenHandler called itself in a loop.',
                    1620737618
                );
            }
